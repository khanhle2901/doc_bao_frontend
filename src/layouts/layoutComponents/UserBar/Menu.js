import { faAddressCard, faArrowRightFromBracket, faRightToBracket, faUserPlus } from '@fortawesome/free-solid-svg-icons'
...
import { setCookieToken, validateEmail } from '../../../optionalFunction'
...
function Menu() {
        ...
        <div className={cx('form')}>
          ...
          <div>
            <input
              ...
              minLength='6'
              placeholder=' '
              onChange={(e) => dispatch({ type: 'UPDATE_LOGIN_PASSWORD', payload: e.target.value })}
            />
        ...
        <div className={cx('form')}>
          <div>
            <input
              ...
              id='signup-name'
              placeholder=' '
              onChange={(e) => dispatch({ type: 'UPDATE_SIGNUP_NAME', payload: e.target.value })}
            />
          ...
          <div>
            <input
              ...
              id='signup-email'
              placeholder=' '
              onChange={(e) => dispatch({ type: 'UPDATE_SIGNUP_EMAIL', payload: e.target.value })}
            />
          ...
          <div>
            <input
              ...
              placeholder=' '
              type={'password'}
              onChange={(e) => dispatch({ type: 'UPDATE_SIGNUP_PASSWORD', payload: e.target.value })}
            />
          ...
          <div>
            <input
              ...
              placeholder=' '
              type={'password'}
              onChange={(e) => dispatch({ type: 'UPDATE_SIGNUP_REPASSWORD', payload: e.target.value })}
            />
    ...
    signUpSuccess: (
      <div className={cx('signup-success')}>
        ...
        <p className={cx('message')}>
          Chúng tôi đã gửi một email xác nhận đăng ký đến Email của bạn. Vui lòng vào hộp thư, nhấn vào link verify để
          hoàn thành đăng ký.
        </p>
      </div>
    ),
  ...
  const handleOkModal = () => {
    switch (state.modalAction) {
      ...
      case 'SIGN_UP':
        ...
        } else if (!validateEmail(state.signUpData.email.trim()) || state.signUpData.email.length > 50) {
          dispatchUpdateErrMessage('Email không hợp lệ!', 1500)
  ...
  return (
    <div className={cx('menu-wrapper')}>
      {state.modalAction && (
        <Modal onCancel={handleCancelModal} onOk={handleOkModal} onClick={handleModalClick} isLoading={state.isLoading}>
          {renderChildModal()}
