...
function CategoryTrash() {
  ...
  const handleOk = async () => {
    if (currentAction.type === 'force-delete') {
      const response = await axiosCT.delete('/category/force', { id: currentAction.id })
    ...
    if (currentAction.type === 'restore') {
      const response = await axiosCT.patch('/category', { id: currentAction.id, status: 0, name: currentAction.name })
      setCurrentAction((prev) => ({}))
          ...
          {currentAction.type === 'force-delete' && (
            <>
              <p className={cx('confirm')}>
                Có chắc chắn muốn xóa <span className={cx('category-name')}>{currentAction.name}</span> vĩnh viễn?
              </p>
              <p className={cx('warning')}>Sau khi xóa không thể khôi phục lại!</p>
            </>
          )}
        ...
        <div className={cx('trashes')}>
          {categoriesTrash.map((category) => (
            <CategoryItem
              key={category.id}
              name={category.name}
              onEdit={() => setCurrentAction((prev) => ({ id: category.id, name: category.name, type: 'restore' }))}
              onDelete={() => {
                setCurrentAction((prev) => ({ id: category.id, name: category.name, type: 'force-delete' }))
              }}
              trash
            />
          ))}
