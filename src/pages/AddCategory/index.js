...
function AddCategory() {
  ...
  const handleOk = async () => {
    ...
    if (currentAction.type === 'delete') {
      const response = await axiosCt.delete('/category', { id: currentAction.id })
      setCurrentAction((prev) => ({}))
  ...
  return (
    <>
      ...
      {Object.keys(currentAction).length !== 0 && currentAction.type === 'update' && (
      ...
      {Object.keys(currentAction).length !== 0 && currentAction.type === 'delete' && (
        <Modal onCancel={handeCancel} onOk={handleOk}>
          <p className={cx('confirm-mes')}>
            Xác nhận xóa danh mục <span className={cx('category-name')}>{currentAction.name}</span> vào thùng rác?
          </p>
        </Modal>
      )}
      <div className={cx('wrapper')}>
        ...
        <div>
          ...
          />
          <button className={cx('btn-add-category')} onClick={handleAddCategory}>
            Thêm
          </button>
        </div>
          ...
          <div className={cx('categories')}>
            {categories.map((category) => (
              <CategoryItem
                key={category.id}
                name={category.name}
                onEdit={() => setCurrentAction((prev) => ({ id: category.id, name: category.name, type: 'update' }))}
                onDelete={() => setCurrentAction((prev) => ({ id: category.id, name: category.name, type: 'delete' }))}
              />
              ...
              //     <button
              //       className={cx('btn-category-update')}
              //       onClick={(e) => {
              //         console.log(e)
              //         setCurrentAction((prev) => ({ id: category.id, name: category.name, type: 'update' }))
              //       }}
              //     >
              //       <FontAwesomeIcon icon={faPenToSquare} />
              //     </button>
              //     <button
              //       className={cx('btn-category-delete')}
              //       onClick={() =>
              //         setCurrentAction((prev) => ({ id: category.id, name: category.name, type: 'delete' }))
              //       }
              //     >
        ...
        <Link to={routers.categoryTrash} className={cx('category-trash')}>
          Thùng rác
        </Link>
      </div>
    </>
